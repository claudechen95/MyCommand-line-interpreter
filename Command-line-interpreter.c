...
// handle FG job and BG job
void FGBGHelper(struct cmdline_tokens tok){
        int jid,pid;
        struct job_t* job;
        char buf[MAXLINE];
        if (tok.argv[1][0] == '%'){
...
//handle Jobs
void JobsHelper(struct cmdline_tokens tok){
    int fd;
        if(tok.outfile !=NULL){
...
eval(char *cmdline) 
{
    ...
    if (bg == -1) /* parsing error */
        ...
        return;
    else if (tok.builtins == BUILTIN_QUIT)exit(0);
        
    else if (tok.builtins == BUILTIN_JOBS)JobsHelper(tok);
